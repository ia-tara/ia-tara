...
\section{Úvod}
\textbf{TARA (Threat Analysis and Risk Assessment)} je systematický proces identifikácie, analýzy a hodnotenia hrozieb a rizík, ktoré môžu ovplyvniť bezpečnosť systému, produktu alebo služby. Cieľom analýzy je nájsť potenciálne zraniteľnosti, identifikovať útoky, ktoré by mohli tieto zraniteľnosti využiť, určiť výsledný dopad takýchto útokov a zistiť, ako je možné sa proti nim efektívne brániť.
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item Definícia jeho funkcií, aké má hranice, vzťah k ostatným častiam systému.
    ...
    \begin{itemize}
        ...
        \item Existujú štyri typy damage scenarios: \uv{safety}, \uv{financial}, \uv{personal}, \uv{operational}
    \end{itemize}
...
\begin{description}
    \item[ATT\&CK Enterprise] sa zameriava na tradičné business IT prostredie -- počítače, servery, cloud, identity, siete. Obsahuje techniky, ktoré útočníci používajú pri útoku na firemné informačné systémy (phishing, privilege escalation, exfiltrácia dát, atď.).
    ...
    \item[ATT\&CK ICS (Industrial Control Systems)] je zameraný na riadenie priemyselných procesov -- bezpečnostné kontroléry, priemyselné protokoly, atď. Techniky obsahujú napríklad útoky na fyzické procesy, manipuláciu signálov, narušenie prevádzky alebo bezpečnostných mechanizmov. Je veľmi blízka oblasti embedded systémov a kritickej infraštruktúry.
\end{description}

\begin{itemize}
    \item Mapuje techniky (typy útokov) na taktiky (ciele útokov). Technika môže patriť do niekoľkých taktík.
...
\textbf{CWE} sa zaoberá najmä implementačnými/návrhovými slabinami softvéru, ktoré je potom možné využiť na napadnutie softvéru.
...
V závislosti od hĺbky môže mať každý uzol typ \uv{Pillar}, \uv{Class}, \uv{Base} alebo \uv{Variant}. Väčšina konkrétnych zraniteľností sú Base alebo Variant.
...
\begin{enumerate}
    \item Navigácia -- klasická všeobecne zaužívaná navigácia v hornej časti okna -- obsahuje všetky základné aj pokročilé funkcie na správu projektov, nastavení a nastavenie samotnej aplikácie.
    \item Riadok s nástrojmi -- určený pre nástroje používané pri práci s rôznymi časťami aplikácie. Tieto nástroje sa môžu meniť v závislosti od aktuálne otvoreného okna.
    \item Navigácia v ľavej časti obrazovky -- obsahuje položky \textbf{Definition} na definíciu samotného projektu. Tu sa vytvárajú itemy, damage scenarios, threat scenarios, attack paths a prepojenia medzi nimi. \textbf{Analysis} slúži na vyhodnotenie rizík. \textbf{Resolution} slúži na návrhy riešenia a rôzne iné poznámky.
    ...
    \item Console -- umožňuje používateľovi zadávať príkazy a zobrazuje históriu vykonaných akcií.
    \item Details -- zobrazuje detaily prvku, ktorý je práve vybraný. Napríklad pri analýze môže používateľ kliknúť na damage scenario a v detailoch by sa mu o ňom zobrazili bližšie informácie. Ak by chcel používateľ všetky informácie k danému prvku alebo by ho chcel modifikovať, mal by mať možnosť prvok jednoducho otvoriť aj ako okno v hlavnej časti aplikácie.
\end{enumerate}
\subsection{Definovanie systému}
Na analýzu systému je najskôr potrebné definovať jeho časti, dáta, ktoré používajú, a vzťahy medzi nimi.
Na to bude možné využiť systémový diagram alebo jednotlivé tabs pre konkrétne položky (komponenty, dátové entity).
...
Tiež môže obsahovať \textbf{dátové entity} a využívané \textbf{technológie}.

Pre komponenty budú definované vzťahy parent-child, pomocou ktorých bude možné jednoduchšie modelovať a agregovať hrozby na komponentoch.
Ďalším vzťahom bude vzťah komunikácie medzi komponentmi, kde jeden komponent môže komunikovať s ďalšími komponentmi. Pri tomto vzťahu je možné pre každý jednotlivý vzťah zadefinovať jeho názov.

Prvým/základným komponentom bude tzv. systémový komponent, ktorý bude obsahovať všetky komponenty projektu ako children.
\subsubsection{Dátové entity}
Dátové entity slúžia na reprezentáciu dát, ktoré používajú komponenty.
Každá dátová entita bude mať svoje jedinečné ID, názov a prípadný popis dát, ktoré reprezentuje.
...
Súčasťou aplikácie bude aj preddefinovaný zoznam týchto technológií, ale používateľ bude mať možnosť definovať vlastné, ak je to potrebné.
\begin{center}
...
Po prejdení všetkých komponentov a dátových entít a určení hrozieb sa automaticky vytvoria kostry scenárov poškodenia pre všetky označené komponenty.
Používateľ ich následne môže upraviť, napríklad upraviť, aká časť CIA je postihnutá týmto konkrétnym scenárom poškodenia.
Tiež môže doplniť položku impact scale, ktorá určuje počet zariadení, ktoré môžu byť zasiahnuté, a položku impact, ktorá definuje vplyv scenára na safety, financial, operational a privacy aspekty.

\subsubsection{Asistent scenárov hrozieb}
Asistent scenárov hrozieb bude využívať zoznam definovaných hrozieb (budú existovať predvolené hrozby, ale používateľ bude môcť dodefinovať vlastné), ktoré bude posudzovať na komponentoch/dátových entitách, kde boli určené scenáre poškodenia.

Následne budú scenáre hrozieb automaticky priradené k scenárom poškodenia, čím bude mať používateľ zľahčenú prácu.
...
Asistent scenárov tiež využíva zoznam definovaných hrozieb a používateľom definované scenáre hrozieb.
Pre každý scenár hrozby je vytvorený krok útoku, ktorý obsahuje informácie o tom, akú hrozbu spôsobuje, na akom komponente/dátovej entite sa nachádza.

Používateľ môže dodefinovať controls, ktoré dokážu zmierniť hrozbu alebo jej predísť, prípadne prípravný attack step, ktorý musí byť vykonaný pre fungovanie tohto kroku.
...
Pri ich definícii je potrebné uviesť názov, komponent alebo dátovú entitu, na ktorú sa aplikujú, a aké scenáre hrozby a kroky útoku mitigujú alebo na ktoré sa aplikujú.
...
Výsledok analýzy je následne možné exportovať ako dokument, ktorý bude obsahovať informácie o systéme, scenároch poškodenia, scenároch hrozieb aj krokoch útoku.
Tiež bude obsahovať control opatrenia a spôsob, akým sú aplikované na jednotlivé kroky útoku/scenáre hrozieb, kde bude jasne vidieť, ktoré z nich boli alebo neboli \uv{opravené} opatreniami.
...
Prvou možnosťou bude využitie nástrojov (implementované pomocou LangChain Tools), ktorými bude možné vytvoriť komponenty/dátové entity napísaním inštrukcií LLM (napr. \uv{vytvor ECU komponent, ktorý obsahuje 3 data entity a má podkomponent...}), čím sa značne zjednoduší proces modelovania systému.

Ďalšou možnosťou bude vykonávať analýzu nad vytvoreným systémom, kde bude mať model (ak to používateľ povolí) prístup ku kontextu systému a bude môcť analyzovať jeho časti podľa štandardu.

\subsection{Backend}
Backend server bol navrhnutý s použitím technológií \texttt{Django} spolu s \texttt{Django REST Framework} (\url{https://www.django-rest-framework.org/}). Okrem manipulácie s databázou mal server zabezpečovať autorizáciu a vnútornú logiku aplikácie.

Nasledujúci zoznam endpointov je pôvodný návrh API z prototypovej fázy. Nejde o úplnú dokumentáciu finálnej implementácie. Výsledné API sa počas implementácie rozšírilo o projekty, JWT autorizáciu, \texttt{project\_id}, damage scenario concerns, control groups, risk treatment, cybersecurity goals, MITRE endpointy, report a LLM asistenta. Finálny stav je preto zložitejší a je opísaný neskôr v kapitole \uv{Výsledná implementácia}, najmä v časti \uv{Backend API a synchronizácia vzťahov}.
...
Pri vývoji využívame doplnkové knižnice, najmä shadcn, Tailwind a Lucide, pomocou ktorých vytvárame konzistentné, moderné a vizuálne zjednotené prostredie. Pri návrhu homepage sme sa inšpirovali dizajnom z nástroja Figma zobrazeného na \nameref{figma1}.
...
Pôvodný frontend a backend vznikali ako samostatné prototypy. Vo finálnej fáze boli zlúčené do jedného repozitára, ktorý obsahuje adresáre \texttt{frontend/}, \texttt{backend/} a \texttt{documentation/}.

\section[Backend -~druhá iterácia]{Backend -- druhá iterácia}
...
\subsection{Autentizácia a autorizácia}
...
\subsection{Príklad použitia backendu}
Nasleduje príklad použitia backendu, vrátane registrácie používateľa, s použitím softvéru \texttt{Postman}:
\begin{enumerate}
    ...
    \end{figure}
    \item Prihlásenie používateľa -- v odpovedi je token, ktorý sa použije v každej ďalšej požiadavke
    \begin{figure}[H]
        \centering
        \includegraphics[width=\textwidth]{backend/scr2.png}
    \end{figure}
    \item Vytvorenie projektu. Na druhom obrázku je zobrazené, že token získaný v minulom kroku je v hlavičke \texttt{Authorization}. Bude sa odosielať po celý zvyšok relácie. Vytvorený projekt je takto asociovaný s daným používateľom a akékoľvek operácie s projektom budú musieť byť iba od tohto používateľa.
    \begin{figure}[H]
        \centering
    ...
    \end{figure}
    \item Vytvorenie komponentu. Tu je dôležitá hlavne nová položka: \texttt{project\_id}. Komponent je teda asociovaný s práve jedným projektom. Táto položka spolu s hlavičkou \texttt{Authorization} bude v každej ďalšej požiadavke pracujúcej s projektom.
    \begin{figure}[H]
        \centering
    ...
    \end{figure}
    \item Získanie detailov konkrétneho komponentu. V tomto prípade je v query URL číslo komponentu (tak ako v predošlej verzii API) a následne je v query parametroch číslo projektu (tak ako v predošlej požiadavke)
    \begin{figure}[H]
        \centering
        \includegraphics[width=\textwidth]{backend/scr7.png}
    \end{figure}
    \item Vytvorenie damage scenaria. Súčasťou tela požiadavky je \texttt{component\_id} a \texttt{project\_id}. Daný damage scenario je tak asociovaný s práve jedným komponentom, ktorý je v práve jednom projekte, pričom ako už bolo spomenuté, vlastníctvo daného projektu používateľom je zabezpečené hlavičkou \texttt{Authorization}. Na rovnakom princípe fungujú aj ostatné POST operácie, ako vytvorenie attack path alebo threat scenario.
    \begin{figure}[H]
        \centering
        \includegraphics[width=\textwidth]{backend/scr8.png}
    \end{figure}
    \item Získanie detailov konkrétneho komponentu. Tento krok je rovnaký ako pred dvoma krokmi -- je tu iba pre ukážku, akým spôsobom sa damage scenario vytvorený v predošlom kroku pridal do komponentu.
    \begin{figure}[H]
        \centering
...
\subsection{Beh a nasadenie aplikácie}

Projekt je možné spustiť dvoma spôsobmi. Pri vývoji je výhodné spúšťať frontend a backend oddelene: Django beží na porte \texttt{8000} a Vite vývojový server na porte \texttt{5173}. V tomto režime frontend automaticky volá API na adrese \texttt{http://localhost:8000/api}.

Pre reálne nasadenie je pripravený jeden Docker kontajner. Docker build najprv zostaví React/Vite frontend poskytovaný pod cestou \texttt{/static/}, potom výsledný \texttt{dist} adresár skopíruje do výsledného image. Django následne poskytuje statické frontend súbory cez \texttt{/static/} a samotnú single-page aplikáciu na koreňovej adrese \texttt{/}. Backend API ostáva dostupné pod \texttt{/api/} a Django admin pod \texttt{/admin/}. Potrebná Docker Compose konfigurácia preto obsahuje iba službu \texttt{app} a volume pre SQLite databázu.
...
Rozhranie hlavnej aplikácie má stabilné rozloženie. Hore je menubar a toolbar, vľavo je navigácia podľa aktuálneho režimu, v strede hlavný pracovný priestor, dole je konzola a vpravo detailový panel. V hornom menu je okrem položiek \texttt{File} a \texttt{Help} aj tlačidlo \texttt{Login}, ktoré vymaže aktuálnu session, project ID a tokeny, a tým vráti aplikáciu späť na prihlasovaciu obrazovku. Toto je najjednoduchší spôsob, ako počas behu aplikácie prepnúť používateľa. Toto rozloženie nadväzuje na pôvodný návrh aplikácie z kapitoly \uv{Návrh}, ale vo výslednej implementácii sú jednotlivé časti už funkčne prepojené.

\subsection{Autentizácia, projekty a prístupové práva}

Autentizácia je implementovaná pomocou JWT tokenov cez knižnicu \texttt{djangorestframework-simplejwt}. Backend poskytuje endpointy \texttt{/api/register/}, \texttt{/api/token/} a \texttt{/api/token/refresh/}. Registrácia vytvorí nového používateľa v štandardnom Django modeli \texttt{User}. Vo frontende je registrácia dostupná priamo z prihlasovacej obrazovky cez možnosť \uv{Create a new user}. Používateľ zadá meno, heslo a prípadne email. Po úspešnej registrácii frontend zavolá bežné prihlásenie, takže používateľ nemusí manuálne prechádzať späť na login. Prihlásenie vráti access token a refresh token. Frontend access token posiela v hlavičke:
...
Backend používa pomocnú funkciu \texttt{check\_project\_access}. Tá sa volá v jednotlivých endpointoch a kontroluje, či existuje projekt s daným ID a či jeho \texttt{owner} je aktuálne prihlásený používateľ. Ak projekt neexistuje alebo nepatrí danému používateľovi, backend vráti chybu \texttt{403}. Toto je použité konzistentne pri komponentoch, scenároch, control skupinách, rizikách, reportoch aj LLM asistentovi.

Vo frontende sa projekt vyberá na stránke \texttt{ProjectSelector}. Tá po prihlásení načíta zoznam projektov cez \texttt{/api/projects/}. Používateľ môže projekt vybrať alebo rovno vytvoriť nový. ID a meno vybraného projektu sa uložia do \texttt{sessionStorage}. Po otvorení projektu komponent \texttt{ProjectPage} zavolá \texttt{loadProjectState}, čím sa naraz načíta celý projektový model a vypočítané riziká.

Toto riešenie má praktickú výhodu: používateľ nemusí v jednotlivých obrazovkách opakovane vyberať projekt. Frontend automaticky prikladá \texttt{project\_id} ku každému relevantnému API volaniu. Backend napriek tomu nikdy neverí iba frontendu, ale vlastníctvo projektu kontroluje sám.
...
Technológie samy o sebe netvoria riziko, ale pomáhajú opísať prostredie, v ktorom komponent funguje. V LLM kontexte a v asistentoch sa používajú ako nápoveda pre to, aké typy hrozieb alebo útokov môžu byť relevantné. Vo frontende sa technológie zobrazujú v strome pod komponentmi, v grafickom pohľade ako samostatné karty a v detailoch ako vzťahy.
...
Na strane backendu sú k dispozícii aj endpointy \texttt{/api/mitre/tactics/} a \texttt{/api/mitre/techniques/}. Tieto endpointy získavajú dáta z MITRE ATT\&CK STIX zdroja a poskytujú ich frontendu. Vo formulári pre threat class a attack step sa tak dajú používať MITRE taktiky a techniky bez toho, aby ich používateľ musel manuálne prepisovať.
...
V modeli je pole \texttt{impact\_scale}, ale vo výslednej implementácii sa nastavuje automaticky v metóde \texttt{save}. Hodnota \texttt{impact\_scale} je maximum zo štyroch impact kategórií. To znamená, že používateľ nemusí ručne udržiavať dve konzistentné hodnoty. Stačí nastaviť safety, financial, operational a privacy impact a backend vypočíta celkový impact level.
...
Používateľ môže karty presúvať po ploche. Pozície sa ukladajú do \texttt{localStorage} pod kľúčom odvodeným od projektu. V menu \texttt{File} je možné layout uložiť do JSON súboru a neskôr ho načítať. Po načítaní layoutu frontend nastaví uložené pozície na existujúce uzly.

Graf podporuje aj filtrovanie. Používateľ môže filtrovať podľa názvu a podľa typu objektu. Filtrovanie ovplyvňuje iba zobrazenie grafu, nie dáta v projekte. Pri kliknutí na kartu sa vybraný objekt nastaví ako \texttt{selectedItem} a otvorí sa v pravom detailovom paneli. Ak používateľ vyberie objekt v strome, graf sa naň vie automaticky centrovať.
...
Týmto spôsobom môže používateľ model skladať vizuálne. Nemusí si pamätať ID objektov ani manuálne editovať všetky zoznamy vzťahov. Aplikácia zároveň zabraňuje duplicitnému prepojeniu a nepovolí vzťahy, ktoré dátový model nepodporuje.
...
Tento istý princíp je použitý aj v detailovom paneli. Vzťahy nie sú zobrazené iba ako text. Sú to klikateľné značky, ktoré umožňujú rýchlo prejsť na súvisiaci objekt. Vedľa zoznamu vzťahov je tlačidlo na pridanie ďalšieho vzťahu. Používateľ teda môže začať napríklad na komponente, vytvoriť k nemu attack step, z attack stepu vytvoriť threat scenario, k threat scenariu pridať damage scenario a potom vrátiť sa k attack stepu a doplniť controls.
...
V rozbalenom detaile risku je aj editor risk treatment. Používateľ môže vybrať rozhodnutie Avoid, Reduce, Share alebo Accept a dopísať rationale. Uloženie zavolá endpoint \texttt{/api/risk\_treatment/}. Po uložení sa riziká znovu načítajú, aby tabuľka zobrazovala aktuálny treatment.
...
Control skupiny sú dostupné z toolbaru cez tlačidlo \texttt{Controls}. Dialóg zobrazuje dve vstavané skupiny \texttt{All Controls} a \texttt{No Controls}, a potom všetky používateľské skupiny z databázy. Používateľ môže skupinu vybrať, vytvoriť novú, upraviť existujúcu alebo ju zmazať.
...
Na backende sa control groups ukladajú v modeli \texttt{ControlGroup}. Report generátor ich používa na samostatnú tabuľku \uv{Control Scenarios per Risk}. Tam sa pre každý risk ukáže základný risk level a risk level pri jednotlivých control skupinách. To je užitočné najmä pri porovnávaní alternatívnych mitigácií. Používateľ vie napríklad ukázať, že minimálna sada opatrení zníži RL z 4 na 3, ale plná sada opatrení zníži RL z 4 na 2.
...
Návrhy sa zobrazujú v tabuľkovom zozname. Pri každom návrhu je stav: Proposed, Accepted, Rejected alebo Creating. Používateľ môže návrh prijať, odmietnuť, obnoviť odmietnutý návrh alebo použiť \texttt{Apply All}. Pri jednom assete sa dá použiť aj \texttt{Apply All} iba pre jeho tri CIA časti.
...
Source tag v popise nie je viditeľný prvok pre koncového používateľa, ale používa sa na rozpoznanie, či už bol návrh vytvorený. Asistent vie nájsť damage scenario, ktoré obsahuje rovnaký source tag, a potom ho zobrazí ako \uv{Created}. Zároveň vie nájsť aj existujúce scenáre, ktoré už pokrývajú rovnaký komponent a CIA časť, a vtedy namiesto \uv{Accept} zobrazí možnosť \uv{Add another}. Tým sa znižuje riziko, že asistent bude vytvárať duplicitné scenáre bez upozornenia.
...
LLM asistent si tiež pamätá stav návrhov. Ak používateľ návrh prijal, odmietol alebo jeho uloženie zlyhalo, táto informácia sa posiela späť do backendu ako \texttt{proposalMemory}. Systémový prompt explicitne hovorí modelu, aby neopakoval už prijaté, odmietnuté alebo čakajúce návrhy, pokiaľ používateľ nepožiada o ich revíziu. Tým sa zlepšuje použiteľnosť pri konverzačnej práci typu \uv{analyzuj model}, \uv{navrhni chýbajúce controls}, \uv{pokračuj}.
...
\begin{enumerate}
    \item Používateľ sa prihlási a otvorí projekt \uv{Auto security}.
    ...
    \item V asistentovi Asset Identification pre komponenty, prípadne existujúce data entities, prijme návrhy CIA strát. Tým sa vytvoria prvé threat scenarios a damage scenarios.
...
Finálna aplikácia poskytuje ucelené prostredie na tvorbu TARA analýzy. Používateľ v nej vie spravovať projekty, definovať systémové komponenty a technológie, vytvárať damage scenarios, threat scenarios a attack steps, dopĺňať controls, počítať riziká, porovnávať control skupiny, zapisovať risk treatment, definovať cybersecurity goals a exportovať report.
