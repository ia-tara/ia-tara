...
V závislosti od hĺbky, každý uzol môže mať typ \uv{Pillar}, \uv{Class}, \uv{Base}, alebo \uv{Variant}. Väčšina konkrétnych zraniteľností sú Base alebo Variant.
...
\begin{enumerate}
    ...
    \item Details - Zobrazuje detaily prvku, ktorý je práve zakliknutý. Napríklad pri analýze môže používateľ kliknúť na damage scenario a v detailoch by sa mu o ňom zobrazili bližšie informácie. Ak by chcel používateľ všetky informácie k danému prvku alebo by ho chcel modifikovať, mal by mať možnosť prvok jednoducho otvoriť aj ako okno v hlavnej časti aplikácie.
\end{enumerate}
...
Na toto bude možné využiť systémový diagram alebo jednotlivé tabs pre konkrétne položky (komponenty, dátové entity).
...
Ďalším vzťahom bude vzťah komunikácie medzi komponentmi, kde jeden komponent môže komunikovať s ďalším(i), pri tomto vzťahu je možné pre každý jednotlivý vzťah zadefinovať jeho názov.
...
Súčasťou aplikácie bude aj preddefinovaný list týchto technológií, ale používateľ bude mať možnosť definovať vlastné, ak je to potrebné.
\begin{center}
...
Asistent scenárov hrozieb bude využívať list definovaných hrozieb (budú existovať default hrozby, ale používateľ bude môcť dodefinovať vlastné), ktoré bude posudzovať na komponentoch/dátových entitách, kde boli určené scenáre poškodenia.
...
Asistent scenárov tiež využíva list definovaných hrozieb a používateľom definované scenáre hrozieb.
...
Pri ich definícii je potrebné uviesť názov, komponent alebo dátovú entitu, na ktorú sa aplikujú, a na aké scenáre hrozby a kroky útoku mitigujú/sa aplikujú.
Tiež je potrebné zadefinovať feasibility tabuľku, ktorá bude definovať feasibility hodnoty po aplikovaní control.

Podobne ako pri technológiach bude existovať list preddefinovaných controls (napr. symetrické šifrovanie, asymetrické šifrovanie), ktoré bude používateľ môcť použiť.
...
\subsection{Backend}
Backend server bude implementovaný použitím technológií \texttt{django} spolu s \texttt{rest framework} (\url{https://www.django-rest-framework.org/}). Okrem manipulácie s databázou bude server zabezpečovať autorizáciu a vnútornú logiku aplikácie. Nižšie sú popísané endpointy backend servera slúžiace na komunikáciu s frontendom. Na týchto endpointoch bude server prijímať alebo posielať JSON dáta.  Pri každom z nižšie uvedených endpointov je aj príklad dát, ktoré server očakáva (pre POST požiadavky) alebo odosiela (pre GET požiadavky). Uvedené sú iba názvy JSON polí, ich hodnoty sú vynechané. \\
...
V tejto iterácii boli dokončené všetky endpointy, boli pridané projekty a ostatné CRUD operácie pre všetky databázové objekty. Tiež bola pridaná autentizácia a autorizácia používateľa pomocou JWT.
...
\begin{enumerate}
    ...
    \end{figure}
    \item Vytvorenie projektu. Na druhom obrázku je zobrazené, že token získaný v minulom kroku je v hlavičke \texttt{Authorization}. Bude sa odosielať po celý zvyšok sedenia. Vytvorený projekt je takto asociovaný s daným používateľom, a akékoľvek operácie s projektom budú musieť byť iba od tohto používateľa.
    \begin{figure}[H]
        \centering
    ...
    \end{figure}
    \item Vytvorenie damage scenaria. Súčasťou tela požiadavky je \texttt{component\_id} a \texttt{project\_id}. Tým pádom je daný damage scenario asociovaný s práve jedným komponentom, ktorý je v práve jednom projekte, pričom ako už bolo spomenuté, vlastníctvo daného projektu používateľom je zabezpečené hlavičkou \texttt{Authorization}. Na rovnakom princípe fungujú aj ostatné POST operácie, ako vytvorenie attack path alebo threat scenario.
    \begin{figure}[H]
        \centering
...
V nasledujúcich častiach je popísaná výsledná aplikácia tak, ako funguje po dokončení. Popis je písaný z pohľadu práce používateľa, ale pri každej väčšej funkcionalite je zároveň uvedené, ako je riešená vnútri aplikácie a ako sú jednotlivé časti medzi sebou prepojené.

\subsection{Celkový tok práce v aplikácii}

Po otvorení aplikácie sa používateľ najprv prihlási. Prihlásenie vyžaduje adresu backend servera, meno a heslo. Frontend predvolene očakáva backend na rovnakom hostiteľovi na porte \texttt{8000}, ale používateľ môže adresu zmeniť. Ak používateľ ešte nemá účet, môže na tej istej obrazovke prepnúť jednoduchý režim vytvorenia nového používateľa. Po registrácii sa frontend rovno pokúsi prihlásiť s novým účtom. Po úspešnom prihlásení backend vráti dvojicu JWT tokenov -- \texttt{access} a \texttt{refresh}. Frontend si ich uloží do \texttt{sessionStorage} a následne ich používa pri každej požiadavke na backend.
...
Source tag v popise nie je viditeľná funkcionalita pre koncového používateľa, ale používa sa na rozpoznanie, či už bol návrh vytvorený. Asistent vie nájsť damage scenario, ktoré obsahuje rovnaký source tag, a potom ho zobrazí ako \uv{Created}. Zároveň vie nájsť aj existujúce scenáre, ktoré už pokrývajú rovnaký komponent a CIA časť, a vtedy namiesto \uv{Accept} zobrazí možnosť \uv{Add another}. Tým sa znižuje riziko, že asistent bude vytvárať duplicitné scenáre bez upozornenia.
...
Keď používateľ vytvorí nový objekt, \texttt{addItem} ho uloží cez API. Ak API vráti objekt s projektom, frontend znovu načíta celý projektový stav. Je to jednoduchšie a bezpečnejšie ako ručne skladať všetky odvodené vzťahy na klientovi. Pri mazaní objektu sa volá \texttt{deleteItem}, ktorý zmaže objekt cez API a potom ho odstráni z lokálneho store.
